...
- **Lucia Auth**: A modern, secure, session-based authentication library. Used for user registration, login, logout, and session management. Integrates with Cloudflare D1 via `@lucia-auth/adapter-d1`.
...
- **WebSockets**: Enables bidirectional, real-time communication between the client (interviewee) and the server (Interview Durable Object), secured by Lucia Auth sessions.
- **Cloudflare AI**:
...
- **Cloudflare D1**: A serverless SQL database used for persisting application data, including `Users`, Lucia Auth `user_sessions` and `user_keys`, `Interview_Types`, `Skills`, and interview session data.
...
- **Request Routing**: Uses Hono to route requests to different parts of the application.
  ...
  - Authentication routes (`/api/v1/auth/*`) are handled by [`src/routes/auth.ts`](../src/routes/auth.ts:1) using Lucia Auth.
...
- **Middleware**:
  ...
  - Integrates Lucia Auth middleware (e.g., defined in [`src/middleware/auth.ts`](../src/middleware/auth.ts:1) or a dedicated `luciaAuth.ts`) for session validation and populating user context (`c.var.user`, `c.var.session`) on protected routes.
...
- **Lucia Auth Initialization**: Initializes the Lucia Auth instance (e.g., in `src/lib/lucia.ts` or `src/auth/lucia.ts`) with the D1 adapter, environment-specific settings (e.g., for cookie security), and user/session attribute definitions. Secrets for Lucia Auth are managed via environment variables.
...
- Facilitates the integration of Lucia Auth middleware for protecting routes and managing user sessions.

### 3.3. Lucia Authentication Service (Conceptual: `src/lib/lucia.ts`, [`src/routes/auth.ts`](../src/routes/auth.ts:1), [`src/middleware/auth.ts`](../src/middleware/auth.ts:1))

- **Core Functionality**: Manages user authentication and session lifecycle using Lucia Auth.
- **Key Endpoints** (handled by [`src/routes/auth.ts`](../src/routes/auth.ts:1)):
  - `POST /api/v1/auth/register`: Handles new user registration. Accepts username and password. Uses Lucia Auth to create a new user and a key (with a hashed password) in the D1 database.
  - `POST /api/v1/auth/login`: Authenticates existing users with username and password. Uses Lucia Auth to validate credentials against the `user_keys` table, create a session in the `user_sessions` table, and set a secure, HTTP-only session cookie.
  - `POST /api/v1/auth/logout`: Invalidates the user's current session in D1 (via Lucia Auth) and clears the session cookie from the client.
  - `GET /api/v1/auth/me`: (Protected by Lucia Auth middleware) Retrieves details (e.g., `userId`, `username`) of the currently authenticated user based on their valid session.
- **Session Management**:
  - Lucia Auth handles the creation, validation, refresh, and invalidation of sessions.
...
- **Password Hashing**: Utilizes strong password hashing algorithms (e.g., Argon2id via `oslo/password`) as configured with Lucia's key management for the `user_keys` table.
- **Database Interaction**: Lucia Auth's D1 adapter (`@lucia-auth/adapter-d1`) manages all direct interactions with the D1 tables:
  - `Users`: Stores user identity (e.g., `user_id`, `username`).
  - `user_sessions`: Stores active user sessions with expiration.
  - `user_keys`: Stores authentication methods for users, primarily username/password with hashed passwords.
- **Middleware Integration** ([`src/middleware/auth.ts`](../src/middleware/auth.ts:1) or similar):
  - Validates incoming session cookies using `lucia.validateSession()`.
  - Populates `context.var.user` and `context.var.session` with validated user and session data.
  - Handles session cookie refresh if necessary.
...
- **Authenticated Access**: The WebSocket upgrade request is first validated by Lucia Auth middleware in the main worker. The authenticated `user_id` is then passed to the Durable Object (e.g., via a custom header like `X-User-Id` on the request forwarded to the DO's `fetch` method).
...
- **Data Persistence**: Manages all database operations for _interview-specific_ data, interacting with the Cloudflare D1 database. User and session data are managed by Lucia Auth.
...
- **Static Assets**: Consists of HTML, CSS, and JavaScript files served directly by Cloudflare Workers.
  - [`public/auth.html`](../public/auth.html): Handles user registration and login, interacting with the Lucia Auth API endpoints.
...
- **Key Tables**:
  - `Users`: Stores user information (e.g., `id` (PK, text), `username` (text, unique)). Managed by Lucia Auth. Schema defined in Lucia Auth plan/migrations.
  - `user_sessions`: Stores active user sessions (e.g., `id` (PK, text), `user_id` (FK to Users), `expires_at` (timestamp)). Managed by Lucia Auth. Schema defined in Lucia Auth plan/migrations.
  - `user_keys`: Stores authentication keys for users (e.g., `id` (PK, text, e.g., `username:<username>`), `user_id` (FK to Users), `hashed_password` (text)). Managed by Lucia Auth. Schema defined in Lucia Auth plan/migrations.
...
### 4.1. Authentication Flow (Lucia Auth)
...
5.  It calls Lucia Auth's functions to:
    - Hash the password.
    - Create a new user in the `Users` table in D1.
    - Create a new key (with hashed password) in the `user_keys` table in D1, linked to the user.
...
4.  [`src/routes/auth.ts`](../src/routes/auth.ts:1) receives the request.
5.  It calls Lucia Auth's functions to:
    - Validate the `username` and `password` against the `user_keys` table in D1.
...
4.  Lucia Auth middleware (or route logic) validates the session.
5.  Lucia Auth's `invalidateSession()` is called, removing the session from the `user_sessions` table in D1.
...
2.  Lucia Auth middleware intercepts the request.
3.  `lucia.validateSession()` is called with the session ID from the cookie.
4.  If the session is valid (exists in D1 and not expired):
    ...
    - If the session was refreshed by Lucia, a new `Set-Cookie` header might be sent.
...
### 4.2. Interview Session Flow (WebSocket with Lucia Auth)

1.  **Authentication**: User logs in via Lucia Auth as described above. A valid session cookie is present in the browser.
2.  **Initiation**:
    ...
    - Client-side JavaScript establishes a WebSocket connection to `wss://<worker-url>/api/v1/interview/<interviewId>/ws`. The browser automatically includes the Lucia session cookie with the WebSocket upgrade request.
3.  **Connection Handling & Authentication**:
    ...
    - **Crucially, before upgrading, the Lucia Auth middleware (or specific logic in the route handler) validates the session cookie.**
      - `lucia.validateSession()` is used.
...
## 5. Interactions Diagram (with Lucia Auth)
...
graph LR
    ...
    subgraph "Cloudflare Worker: API & Routing (Hono)"
        ...
        LuciaAuthRoutes[Lucia Auth Routes (src/routes/auth.ts)]
        LuciaAuthMiddleware[Lucia Auth Middleware (src/middleware/auth.ts)]
    ...
    subgraph "Supporting Services (within Worker)"
        LuciaLib[Lucia Auth Library (src/lib/lucia.ts)]
    ...
    MainWorker -- Route --> LuciaAuthRoutes
    LuciaAuthRoutes -- Uses --> LuciaLib
    LuciaLib -- D1 Adapter --> CF_D1
    LuciaAuthRoutes -- Set/Clear Session Cookie --> ClientAuthHTML

    ClientDashboardHTML -- HTTP Request (Cookie Auth) --> MainWorker
    ClientInterviewHTML -- HTTP Request (Cookie Auth) --> MainWorker
    MainWorker -- Apply Middleware --> LuciaAuthMiddleware
    LuciaAuthMiddleware -- Validate Session (Cookie) --> LuciaLib
    LuciaAuthMiddleware -- Populate c.var.user --> MainWorker

    ClientInterviewHTML -- WebSocket Upgrade (Cookie Auth) --> MainWorker
    MainWorker -- Route to InterviewRoute, Apply Auth --> InterviewRoute
    InterviewRoute -- Validate Session (via Middleware/LuciaLib) --> LuciaLib
...
- **Session Management**: Lucia Auth provides secure session management, including HttpOnly, Secure, and SameSite cookies. Session IDs are cryptographically random.
- **Password Hashing**: Strong password hashing (e.g., Argon2id via `oslo/password`) is used to protect user credentials.
- **CSRF Protection**: While Lucia itself is backend, for frontend forms interacting with POST endpoints (login, register), consider implementing CSRF protection mechanisms if not using a framework that handles it (e.g., SameSite cookies offer some protection, but double-submit cookies or token-based methods can be stronger). Hono might have middleware for this.
...
- **Environment Variables**: All secrets (Lucia Auth secrets, database credentials if any were separate, API keys) MUST be stored as Cloudflare Worker environment variables and NOT hardcoded.
- **HTTPS**: Ensure all communication is over HTTPS (Cloudflare handles this by default for worker subdomains).
- **Durable Object Security**: The `user_id` passed to the Durable Object must be trusted, meaning it's derived from a validated Lucia session at the Worker/gateway level before the request is forwarded to the DO. The DO itself should not re-validate the session cookie unless it has direct access to Lucia and its context, which adds complexity.

## 7. Scalability and Performance

- **Cloudflare Workers & D1**: Inherently scalable due to the serverless nature of Cloudflare's platform.
- **Lucia Auth**: Designed to be lightweight. Performance will largely depend on D1 query performance for session/user/key lookups. Proper indexing on D1 tables (`Users.id`, `Users.username`, `user_sessions.id`, `user_keys.id`, `user_keys.user_id`) is crucial.
...
- **Lucia Auth Features**: Lucia supports various adapters and can be extended for features like OAuth, password reset, email verification if needed in the future.
